...
class GrowingNeuralGas:
    ...
    def fit_network(self, e_b, e_n, a_max, l, a, d, passes=1, plot_evolution=False):
        ...
        print('Global error(on euclidean distance): '+ str(global_error))
        plt.clf()
    ...
    def plot_clusters(self, clustered_data):
        ...
        # ax.set_title('Cluster affectation')
    ...
    def plot_3d_clusters(self, clustered_data):
        ...
        for i in range(number_of_clusters):
            ...
            if len(observations) > 0:
                observations = np.array(observations)
                z_points = observations[:, 0]
                x_points = observations[:, 1]
                y_points = observations[:, 2]
                ax.scatter3D(x_points, y_points, z_points,  cmap='hsv', color=color[i], label='cluster #'+str(i))
    ...
    def compute_global_error(self):
        ...
        for observation in self.data:
            nearest_units = self.find_nearest_units(observation)
            s_1 = nearest_units[0]
            global_error += spatial.distance.euclidean(observation, self.network.nodes[s_1]['vector'])
        return global_error/100